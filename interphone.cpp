...
u8 INTERPHONE::scan_encoder(void)
{
	...
	return dir;
#elif 0
	static uint8_t curr_encoder_b;
	static uint8_t last_encoder_b;
	static uint8_t update = 0;
	static uint8_t last_encoder_b_update = 0;

	if(last_encoder_b_update == 0) {
		if( IP_ENCODER_PINA&&IP_ENCODER_PINB ) {
			update = 0;
			return 0;
		}

		last_encoder_b = IP_ENCODER_PINB;
	}

	if(!IP_ENCODER_PINA) {
		curr_encoder_b = IP_ENCODER_PINB;
		update = 1;
		last_encoder_b_update = 1;
		return 0;
	}

	if(update == 1) {
		update = 0;
		last_encoder_b_update = 0;
		if((last_encoder_b == 0) && (curr_encoder_b)) {
			return 1;
		} else if((last_encoder_b) && (curr_encoder_b == 0)) {
			return 2;
		}
	}
	return 0;
#else
	static uint8_t last_a,cur_a;
	uint8_t dir;
	dir = 0;
	cur_a = digitalRead(IP_ENCODER_A);
	if ((last_a == HIGH) && (cur_a == LOW)) {
		if (digitalRead(IP_ENCODER_B) == HIGH) {
			dir = 1;
		} else {
			dir = 2;
		}
